...
export default function AddShipmentBtn() {
    return (
        <div className="flex items-center gap-x-5">
            ...
            <button
                className="cursor-pointer flex items-center justify-center gap-x-1 w-[175px] hover:opacity-80 transition-all duration-300 ease-in-out h-12 bg-main-primary text-sm p-5 text-white rounded-sm"
                onClick={() => { window.location.href = "/shipping/saveshipment" }}
            >
                Add New Shipping <Plus className="w-4" />
            </button>
        </div>
    )
}